...
public class LempelZiv {
     ...
     */
	 public static String compress(String input) {
	        ...
	        while (cursor < input.length()) {
	            ...
	            while (true) {
	                int match = KMP.search(input.substring(cursor, cursor + length), input.substring(Math.max(0, cursor - windowSize), cursor));
    ...
    public static String decompress(String compressed) {
    	...
    	while (counter < compressed.length()) {
    		...
    		if (compressed.substring(counter,counter+1).equals("[")) {counter++;} // Consider an else that throws exception here
    		...
    		if (compressed.substring(counter,counter+1).equals("|")) {counter++;}
    		...
    		System.out.println(prevMatch.toString());
    		System.out.println(length.toString());
    		System.out.println(terminalChar);
    		
    		// If there was no match --> Just append the terminal char
    		if (prevMatch.toString().equals("0") && length.toString().equals("0")) {dText.append(terminalChar);}
    			...
    			String prevMatchString = dText.substring(dText.length()-prevMatchInt,dText.length()-prevMatchInt + lengthInt);
